...
@testset "$(basename(@__FILE__))" begin
	...
	@testset "2. functions ($array_type arrays)" for (array_type, Array, CellArray, allowscalar) in zip(array_types, ArrayConstructors, CellArrayConstructors, allowscalar_functions)
		...
		@testset "fill!" begin
			allowscalar(true)
			fill!(A, 9);   @test all(A.data .== Array(fill(9.0, size(A.data))))
			fill!(B, 9.0); @test all(B.data .== Array(fill(9,   size(B.data))))
		...
		@testset "getindex / setindex! (GPU kernel programming)" begin
			...
			if array_type == "CUDA"
				...
				A.data.=3;                 @cuda blocks=size(A) matsquare2D_CUDA!(A); synchronize(); @test all(A.data .== Array(fill(9, size(A.data))))
				J.data.=3; J_ref.data.=36; @cuda blocks=size(J) matsquare2D_CUDA!(J); synchronize(); @test CUDA.@allowscalar all(J .== J_ref)
				C.data.=2; G.data.=3;      @cuda blocks=size(C) add2D_CUDA!(C, G);    synchronize(); @test all(C.data .== Array(fill(5, size(C.data))))
			end
			if array_type == "AMDGPU"
				...
				A.data.=3;                 wait(@roc gridsize=size(A) matsquare2D_AMDGPU!(A)); @test all(A.data .== Array(fill(9, size(A.data))))
				J.data.=3; J_ref.data.=36; wait(@roc gridsize=size(J) matsquare2D_AMDGPU!(J)); @test AMDGPU.@allowscalar all(J .== J_ref)
				C.data.=2; G.data.=3;      wait(@roc gridsize=size(C) add2D_AMDGPU!(C, G));    @test all(C.data .== Array(fill(5, size(C.data))))
			end
        end;
