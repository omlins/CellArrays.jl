...
@testset "$(basename(@__FILE__))" begin
	...
	@testset "3. Exceptions ($array_type arrays)" for (array_type, Array, CellArray, Float) in zip(array_types, ArrayConstructors, CellArrayConstructors, FloatDefaultTypes)
        ...
        @test_throws IncoherentArgumentError CellArrays.CellArray{Float,2,0}(similar(A.data, Float16), A.dims)                 # Error: eltype(data) must match eltype(T).
        @test_throws IncoherentArgumentError CellArrays.CellArray{Int32,2,prod(dims)}(similar(B.data, Float), B.dims)          # ...
        @test_throws IncoherentArgumentError CellArrays.CellArray{T_Float,2,0}(similar(C.data, Float16), C.dims)               # ...
        @test_throws IncoherentArgumentError CellArrays.CellArray{T_Int32,2,prod(dims)}(similar(D.data, T_Int32), D.dims)   # ...
        @test_throws IncoherentArgumentError CellArrays.CellArray{T2_Float,2,0}(similar(E.data, Float16), E.dims)              # ...
        @test_throws IncoherentArgumentError CellArrays.CellArray{T2_Int32,2,prod(dims)}(similar(F.data, T2_Int32), F.dims) # ...
        @test_throws IncoherentArgumentError CellArrays.CellArray{T_Float,2,1}(similar(G.data, Float16), G.dims)               # ...
