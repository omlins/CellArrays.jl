...
@inline Base.getindex(A::CellArray{T,N,B,T_array}, i::Int) where {T<:Number,N,B,T_array<:AbstractArray{T,3}}   = T(getindex(A.data::T_array, Base._to_linear_index(A.data::T_array,i,1,1)))    # = T(getindex(A.data::T_array,i,1,1))
@inline Base.getindex(A::CellArray{T,N,B,T_array}, i::Int) where {T,N,B,T_array}                = T(getindex(A.data, Base._to_linear_index(A.data::T_array,i,j,1)) for j=1:length(T))   # = T(getindex(A.data, i, j, 1) for j=1:length(T)) # NOTE:The same fails on GPU if convert is used.
@inline Base.setindex!(A::CellArray{T,N,B,T_array}, x::T, i::Int) where {T<:Number,N,B,T_array} = (A.data[i] = x; return)
@inline Base.setindex!(A::CellArray{T,N,B,T_array}, X::T, i::Int) where {T,N,B,T_array}         = (for j=1:length(T) setindex!(A.data::T_array, X[j], Base._to_linear_index(A.data::T_array,i,j,1)) end; return)    # = (for j=1:length(T) A.data[i,j,1] = X[j] end; return)

# @inline Base.getindex(A::CellArray{T}, i::Int) where {T <: Number}      = T(getindex(A.data, (i, 1, 1)))
# @inline Base.setindex!(A::CellArray{T}, x::T, i::Int) where {T<:Number} = (setindex!(A.data, x, (i, 1, 1)); return)
